...
A planned voice assistant needs a name of its own and would
...
notification title, the name in the onboarding and permission copy, and — once
it is built — the voice assistant.
...
The architecture draws a hard line between a server that is deliberately dumb —
a sync relay, with every read and write local — and the intelligence layer
planned for a Telnyx AI Assistant. The two names now sit on either side of that
line, so which half of the system a sentence is about is legible from the noun
it uses.
