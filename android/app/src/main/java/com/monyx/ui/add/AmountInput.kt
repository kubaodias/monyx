...
class AmountInput private constructor(
    ...
    /**
     * The left-hand side of a pending operation, ready to display.
     *
     * Pressing "+" resets the big number to 0 to take the next operand, which
     * left the running total nowhere on screen: you typed 60 + 40 and the only
     * figures you ever saw were 60, then 40, then 100. Showing "60 +" above the
     * field keeps the sum in view while the second operand is typed.
     */
    fun pendingDisplay(locale: Locale = Locale.getDefault()): String? =
        pendingOperand?.let { format(minorToText(it), locale) }
     ...
     */
    fun display(locale: Locale = Locale.getDefault()): String = format(text, locale)
